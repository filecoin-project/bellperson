...
use crate::nova::prover::ProvingAssignment;
use crate::{Index, LinearCombination};
...
pub struct R1CSShapeAux<F: PrimeField> {
    num_cons: usize,
    num_vars: usize,
    num_inputs: usize,
    A: Vec<(usize, usize, F)>,
...
impl<G: Group> Into<R1CSShape<G>> for R1CSShapeAux<G::Scalar>
where
    G::Scalar: PrimeField,
{
    fn into(self) -> R1CSShape<G> {
        R1CSShape::new(
            self.num_cons,
            self.num_vars,
            self.num_inputs,
